/*
 *     Generated by class-dump 3.3.4 (64 bit).
 *
 *     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2011 by Steve Nygard.
 */

#import <IBFoundation/IBMessageChannel.h>

@interface IBMessageReceiveChannel : IBMessageChannel
{
}

- (void)deliverMessage:(SEL)arg1 toTarget:(id)arg2 withArguments:(id)arg3 result:(id *)arg4;
- (void)runBlockingReceiveLoopNotifyingQueue:(struct dispatch_queue_s *)arg1 notifyingTarget:(id)arg2;
- (void)notifyTarget:(id)arg1 onQueue:(struct dispatch_queue_s *)arg2 ofShutdownWithError:(id)arg3;
- (BOOL)startupReturningError:(id *)arg1;

@end

